...
Compilation proves that an entry is closed, deterministic, nominally typed,
...
not source history, an author decision log, or a second authoring format.

Exports accept the selected transient project, recompile its exact workspace
and entry, verify workspace/closure/build/product lineage, and run target
validation. A target-specific adaptation must be explicit in the existing
export receipt. Silent loss of renderable geometry, animation, or texture data
is forbidden.
