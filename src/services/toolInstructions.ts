...
export const TEXTURE_WORKFLOW_INSTRUCTIONS = [
  ...
  'Prefer the macro tool: texture_pipeline. Use plan for auto-assign + auto-UV with texel-density planning; it avoids resolution growth loops. autoRecover defaults on during paint/facePaint steps and handles overlap/scale/uvUsageId mismatch or missing UVs. Low-level apply_texture_spec/generate_texture_preset autoRecover runs a single auto_uv_atlas + preflight retry.',
  'For material-first painting, use texture_pipeline.facePaint to map materials to presets and target cube faces without manual UV work (requires UV mapping; pair with plan or autoRecover).',
  ...
  'UV overlaps are errors unless the rects are identical. UV scale mismatches are errors. autoRecover runs a plan-based re-UV (auto-split, <=512, max 16 textures) and then repaint.',
