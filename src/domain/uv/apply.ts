...
import { validateUvBounds } from './bounds';
import { validateUvAssignments, type UvAssignmentMessages } from './assignments';
import type { UvBoundsMessages } from './bounds';
...
export type UvApplyMessages = {
  assignments: UvAssignmentMessages;
  bounds: UvBoundsMessages;
  unboundFace: (cubeName: string, face: CubeFaceDirection) => string;
  conflict: (cubeName: string, face: CubeFaceDirection) => string;
  cubeIdNotFound: (id: string) => string;
  cubeNameDuplicate: (name: string) => string;
  cubeNameNotFound: (name: string) => string;
};
...
export const buildUvApplyPlan = (
  ...
  messages: UvApplyMessages
): DomainResult<UvApplyPlan> => {
  const assignmentsRes = validateUvAssignments(assignments, messages.assignments);
  ...
  for (const assignment of assignments) {
    const targets = resolveAssignmentTargets(assignment, cubeById, cubeByName, duplicateNames, messages);
    ...
    for (const target of targets.data) {
      ...
      for (const [faceKey, uv] of faceEntries) {
        ...
        if (textureResolution) {
          const boundsErr = validateUvBounds(uv, textureResolution, undefined, messages.bounds);
          if (boundsErr) return boundsErr;
        }
        const faceRef = resolveUsageFaceRef(usageIndex, target, faceDir);
        if (!faceRef) {
          return fail('invalid_state', messages.unboundFace(target.name, faceDir));
        }
        ...
        if (existing) {
          ...
          if (!same) {
            return fail('invalid_payload', messages.conflict(target.name, faceDir));
          }
        } else {
...
const resolveAssignmentTargets = (
  ...
  if (ids.size === 0 && names.size === 0) {
    return fail('invalid_payload', messages.assignments.targetRequired);
  }
  const targets: Cube[] = [];
  for (const id of ids) {
    if (!cubeById.has(id)) {
      return fail('invalid_payload', messages.cubeIdNotFound(id));
    }
  ...
  for (const name of names) {
    if (duplicateNames.has(name)) {
      return fail('invalid_payload', messages.cubeNameDuplicate(name));
    }
    if (!cubeByName.has(name)) {
      return fail('invalid_payload', messages.cubeNameNotFound(name));
    }
