...
import type { CubeFace, CubeFaceDirection, CubeInstance } from '../../types/blockbench';
import { CUBE_FACE_DIRECTIONS } from '../../shared/toolConstants';
import { computeExpectedUvSize, DEFAULT_UV_POLICY } from '../../domain/uv/policy';
...
export const buildBoxUvLayout = (
  ...
  const resolveSize = (width: number, height: number): { width: number; height: number } => {
    const expected = computeExpectedUvSize({ width, height }, resolution, policy);
