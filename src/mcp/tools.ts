...
const entityAnimationSchema: JsonSchema = {
  type: 'object',
...
const toolSchemas: Record<string, JsonSchema> = {
  ...
  apply_entity_spec: {
    ...
    additionalProperties: false,
    properties: {
      format: { type: 'string', enum: ['geckolib', 'modded_entity', 'optifine_entity'] },
      targetVersion: { type: 'string', enum: ['v3', 'v4'] },
      ensureProject: {
        type: 'object',
        additionalProperties: false,
        properties: {
...
export const MCP_TOOLS: McpToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      'High-level: orchestrates entity workflows (model + textures + animations + triggers) for supported formats. Currently implemented: geckolib (targetVersion v3/v4). Modded/OptiFine are TODO and will return not_implemented. Provide ensureProject to auto-create/reuse a geckolib project, then model/textures/animations/triggers as needed. See bbmcp://guide/entity-workflow via resources/read.',
