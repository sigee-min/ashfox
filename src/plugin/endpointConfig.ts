...
const readFileConfig = (logger?: Logger): Partial<EndpointConfig> => {
...
export const resolveEndpointConfig = (logger?: Logger): EndpointConfig => {
  ...
  const fromFile = readFileConfig(logger);
  const fromEnv = readEnvConfig();
  return {
    host: fromEnv.host ?? fromFile.host ?? defaults.host,
    port: fromEnv.port ?? fromFile.port ?? defaults.port,
    path: fromEnv.path ?? fromFile.path ?? defaults.path
  };
};
