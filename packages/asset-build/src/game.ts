...
import type { GameAssetManifest, RuntimeAsset } from './runtime';
import { digest, json } from './digest';
import { archivePack } from './archive';
export const compileGamePack = async (pack: GameAssetPack, assets: readonly CatalogAsset[], artifacts: readonly Artifact[], encoder?: PackEncoder): Promise<readonly Artifact[]> => {
  const files: Artifact[] = [], runtime: RuntimeAsset[] = [], encoded = new Map<string, Uint8Array>();
  ...
  for (const binding of [...pack.assets].sort((a, b) => a.id < b.id ? -1 : 1)) {
    ...
    const add = (name: string, bytes: Uint8Array): string => {
      const path = `${binding.path}/${name}`; mapped.push({ path, bytes }); return path;
    };
    let detail: Omit<Extract<RuntimeAsset, { kind: 'model' }>, 'files'> |
    ...
    if (source.kind === 'sound' && 'variants' in source.metadata) {
      ...
      for (const variant of source.metadata.variants) {
        ...
        if (pack.audio === 'ogg') {
          if (!encoder) throw new BuildFailure('pack.encoder', 'OGG game assets require a Vorbis encoder');
          let ogg = encoded.get(file.path);
          if (!ogg) { ogg = await encoder.encode(data); encoded.set(file.path, ogg); }
          data = ogg;
        }
        variants.push({ id: variant.id, file: add(`${variant.id}.${pack.audio}`, data),
          durationSeconds: variant.frames / variant.sampleRate, sampleRate: variant.sampleRate, channels: variant.channels });
      }
      ...
      detail = { id: binding.id, kind: 'sprite', image: add(`${source.id}.png`, read(file.path)),
        width: source.metadata.width, height: source.metadata.height, pixelsPerUnit: pack.pixelsPerUnit, filter: pack.spriteFilter };
      ...
      const data = read(file.path), view = new DataView(data.buffer, data.byteOffset, data.byteLength);
      const gltf = JSON.parse(new TextDecoder().decode(data.slice(20, 20 + view.getUint32(12, true)))) as { extensionsRequired?: string[] };
      detail = { id: binding.id, kind: 'model', model: add(`${source.id}.glb`, data),
        requiredExtensions: gltf.extensionsRequired ?? [],
        coordinateSystem: 'gltf2', unitsPerMeter: pack.unitsPerMeter, clips: source.metadata.clips };
    } else throw new BuildFailure('pack.source', source.id + ': unsupported metadata');
    files.push(...mapped);
    runtime.push({ ...detail, files: mapped.map(f => ({ path: f.path, sha256: digest(f.bytes), byteLength: f.bytes.length })) });
  }
