...
import { BuildFailure, type CompiledBundle, type Snapshot } from '../contract';
...
const lockOutput = (directory: string): (() => void) => {
  ...
  try { fs.mkdirSync(lock); }
    ...
    if ((error as NodeJS.ErrnoException).code === 'EEXIST') throw new BuildFailure('output.locked', `Writer lock exists: ${lock}`, 4);
    throw error;
  }
  try {
    ...
    if (entries.length === 0) fs.writeFileSync(owner, marker, { flag: 'wx' });
      ...
      noLinks(owner);
      if (!fs.existsSync(owner) || fs.readFileSync(owner, 'utf8') !== marker) throw new BuildFailure('output.unowned', directory, 3);
    }
    return () => fs.rmSync(lock, { recursive: true });
  } catch (error) { fs.rmSync(lock, { recursive: true }); throw error; }
};
export const withOutputLocks = async <T>(snapshot: Snapshot, action: () => Promise<T>): Promise<T> => {
  const releases: (() => void)[] = [];
  try {
    const directories = [snapshot.config.build.directory, ...snapshot.config.exports.map(e => e.directory), ...(snapshot.config.packs ?? []).map(p => p.directory)].sort();
    for (const directory of directories) releases.push(lockOutput(contained(snapshot.root, directory)));
    return await action();
...
export const publishBundle = (snapshot: Snapshot, bundle: CompiledBundle) => {
  ...
  noLinks(path.join(output, 'bundles')); fs.mkdirSync(path.join(output, 'bundles'), { recursive: true });
  ...
  try {
    ...
    const exports = bundle.catalog.assets.map(asset => {
      ...
      noLinks(path.join(directory, 'bundles')); fs.mkdirSync(path.join(directory, 'bundles'), { recursive: true });
      ...
      try {
        for (const file of asset.files) write(pending, file.path.slice(prefix.length),
        ...
        if (json(listFiles(target)) !== json(expected)) throw new BuildFailure('output.integrity', target, 3);
        for (const file of asset.files) {
          const bytes = fs.readFileSync(contained(target, file.path.slice(prefix.length)));
          if (bytes.length !== file.byteLength || digest(bytes) !== file.sha256) throw new BuildFailure('output.integrity', file.path, 3);
        }
        return { id: asset.id, directory: target };
      } finally { if (fs.existsSync(pending)) fs.rmSync(pending, { recursive: true }); }
    });
    assertSnapshot(snapshot);
    const pointer = path.join(output, `.current-${process.pid}.json`);
    try {
      write(output, path.basename(pointer), json({ format: 'ashfox-current', version: 1, bundleHash: bundle.bundleHash }));
      noLinks(path.join(output, 'current.json'));
      fs.renameSync(pointer, path.join(output, 'current.json'));
    } finally { if (fs.existsSync(pointer)) fs.unlinkSync(pointer); }
    return { requestKey: bundle.receipt.requestKey, bundleHash: bundle.bundleHash, bundlePath: destination,
      catalogPath: path.join(destination, 'catalog.json'), exports };
