...
const root = path.resolve(__dirname, '../..'), temp = fs.mkdtempSync(path.join(os.tmpdir(), 'ashfox-packs-'));
...
const original = JSON.parse(fs.readFileSync(config, 'utf8')); original.packs[0].sounds = [];
...
const run = (command, input = config) => {
  const child = spawnSync(process.execPath, [path.join(__dirname, 'dist/ashfox.cjs'), command, input, '--json'],
    { encoding: 'utf8', timeout: 120000, env: { ...process.env, ASHFOX_FFMPEG_PATH: '/missing/not-needed' } });
...
try {
  write(original);
  const first = success(run('build')), pack = first.exports.find(e => e.id === 'game_pack');
  const files = unzipSync(fs.readFileSync(path.join(pack.directory, 'game_pack.zip')));
  assert.deepEqual(JSON.parse(Buffer.from(files['pack.mcmeta']).toString()).pack.min_format, [88, 0]);
  assert.ok(files['pack.png']);
  const item = JSON.parse(Buffer.from(files['assets/minecraft/items/iron_sword.json']).toString());
  assert.equal(item.model.model, 'minecraft:item/iron_sword');
  const model = JSON.parse(Buffer.from(files['assets/minecraft/models/item/iron_sword.json']).toString());
  ...
  for (const [name, bytes] of Object.entries(files)) assert.deepEqual(Buffer.from(bytes), fs.readFileSync(path.join(pack.directory, 'resource-pack', name)));
  ...
  collision.exports.push({ ...collision.exports.find(e => e.name === 'marker'), name: 'duplicate', directory: 'dist/duplicate' });
  collision.packs[0].models.push('duplicate'); write(collision);
  ...
  legacy.packs[0].metadata = { format: 'legacy', packFormat: 34, description: 'Configured legacy shape' };
  legacy.packs[0].itemDefinitions = 'legacy'; legacy.packs[0].archive = false; legacy.packs[0].models = [];
  ...
  const other = combined.exports.find(e => e.id === 'second_pack').directory;
  assert.ok(fs.existsSync(path.join(other, 'second_pack.zip')));
  assert.ok(fs.existsSync(path.join(other, 'resource-pack/assets/minecraft/items/iron_sword.json')));
  const old = combined.exports.find(e => e.id === 'game_pack').directory;
  ...
  assert.equal(JSON.parse(fs.readFileSync(path.join(old, 'resource-pack/pack.mcmeta'))).pack.pack_format, 34);
  ...
  console.log('declarative packs: exact ZIP, PNG/model links, format configuration, cold build, collision/lock and mirror integrity ok');
