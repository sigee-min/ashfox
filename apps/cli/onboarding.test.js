...
const run = (...args) => spawnSync(process.execPath, [cli, ...args], { cwd: temp, encoding: 'utf8', timeout: 30000,
  env: { ...process.env, ASHFOX_CHROME_PATH: path.join(temp, 'missing-chrome'), ASHFOX_FFMPEG_PATH: path.join(temp, 'missing-ffmpeg') } });
try {
  for (const args of [[], ['--help'], ['help']]) { const result = run(...args); assert.equal(result.status, 0); assert.match(result.stdout, /init <new-folder>/); }
  ...
  assert.equal(JSON.parse(diagnosed.stdout).result.capture.available, false);
  assert.equal(JSON.parse(diagnosed.stdout).result.ogg.available, false);
  ...
  assert.equal(JSON.parse(invalidJson.stdout).ok, false);
  assert.equal(invalidJson.stderr, '');
  assert.notEqual(run('init', 'assets', '--bogus').status, 0);
  assert.equal(run('init', 'assets', '--json').status, 0);
  assert.equal(fs.existsSync(path.join(temp, 'assets/package.json')), false, 'does not create a nested npm project');
  const original = fs.readFileSync(path.join(temp, 'assets/sword.ashfox'));
  assert.notEqual(run('init', 'assets').status, 0);
  assert.deepEqual(fs.readFileSync(path.join(temp, 'assets/sword.ashfox')), original);
  assert.equal(run('export', 'assets/sword.ashfox', '--output', 'sword.png').status, 0);
  ...
  assert.equal(run('export', 'my assets/sword.ashfox', '--output', 'second.png').status, 0);
  for (const args of [['doctor', '--json', '--json'], ['doctor', 'extra'], ['--help', 'extra']]) {
    assert.equal(run(...args).status, 2);
  }
  console.log('CLI onboarding: help/version, optional tools, offline starter and collision preservation pass');
