...
const run = (...args) => {
  const result = spawnSync(process.execPath, [cli, ...args, '--json'], { encoding: 'utf8', timeout: 60000 });
...
try {
  ...
  assert.ok(first.report.result.exports.every(item => item.directory.startsWith(path.join(fs.realpathSync(root), 'build'))));
  ...
  const adapted = spawnSync(process.execPath, [path.join(root, 'assets.mjs'), cli], { encoding: 'utf8', timeout: 60000 });
  assert.equal(adapted.status, 0, adapted.stderr);
  assert.equal(JSON.parse(adapted.stdout).bundleHash, first.report.result.bundleHash);
  ...
  `, path.join(root, 'assets.mjs'), cli], { encoding: 'utf8', timeout: 60000 });
  ...
  const customized = spawnSync(process.execPath, [path.join(root, 'assets.mjs'), cli], { encoding: 'utf8', timeout: 60000 });
  assert.equal(customized.status, 0, customized.stderr);
  assert.ok(JSON.parse(customized.stdout).bundlePath.includes(path.join('build', 'custom', 'compiler')));
  fs.writeFileSync(file, initial);
  const pointer = path.join(root, 'build/assets/compiler/current.json');
  const before = fs.readFileSync(pointer, 'utf8');
  fs.writeFileSync(file, initial.replace("name: 'game-assets'", "name: 'game-assets', unknown: true"));
  assert.notEqual(run('build', file).status, 0);
  assert.equal(fs.readFileSync(pointer, 'utf8'), before);
  fs.writeFileSync(file, "import fs from 'node:fs'; const count = Number(fs.existsSync('count') ? fs.readFileSync('count', 'utf8') : 0) + 1; fs.writeFileSync('count', String(count));\n" + initial.replace("name: 'game-assets'", "name: 'game-' + count"));
  ...
  fs.writeFileSync(outside, "import fs from 'node:fs'; fs.writeFileSync('EXECUTED', 'bad');\n" + initial);
  fs.unlinkSync(file);
  fs.symlinkSync(outside, file);
  assert.equal(run('check', path.join(root, 'asset/items/sword.ashfox')).report.diagnostics[0].code, 'path.symlink');
  ...
  console.log('Executable workspace: grouped starter, adapter, clean rebuild, invalid config and ambiguity pass');
