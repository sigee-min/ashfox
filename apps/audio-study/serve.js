...
const startServer = (port = 3134) => {
  ...
  const start = (request) => {
    if ([...jobs.values()].some((j) => ['running', 'cancelling'].includes(j.state))) throw new Error('Build already running');
    ...
    let output = '', stderr = '';
    const kill = () => { try { if (process.platform !== 'win32') process.kill(-child.pid, 'SIGKILL'); else {
      const killed = spawnSync('taskkill', ['/pid', String(child.pid), '/T', '/F'], { windowsHide: true, timeout: 5000 });
      if (killed.error || killed.status !== 0) child.kill('SIGKILL');
    } } catch { /* Already exited. */ } };
    let join; const joined = new Promise((resolve) => { join = resolve; });
    job.cancel = async () => { if (job.state === 'running') { job.state = 'cancelling'; kill(); await joined; job.state = 'cancelled'; } };
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'POST') {
        ...
        if (cancel) { const job = jobs.get(cancel[1]); if (!job) return json(res, 404, { error: 'Unknown job' }); await job.cancel(); return json(res, 200, job); }
